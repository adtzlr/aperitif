...
#top
ControlledDOF[nodes[:,2]==1,2] = 'Displ-z'
ControlledDOF[nodes[:,2]==1,0] = 'Fixed'
ControlledDOF[nodes[:,2]==1,1] = 'Fixed'

#bottom
ControlledDOF[nodes[:,2]==0,:] = 'Fixed'
#ControlledDOF[nodes[:,2]==0,1] = 'Fixed'
#ControlledDOF[nodes[:,2]==0,2] = 'Fixed'

# Symmetry Planes XY
ControlledDOF[nodes[:,0]==0,0] = 'Fixed'
ControlledDOF[nodes[:,1]==0,1] = 'Fixed'

BC_Labels = ['Fixed','Displ-z']
BC_Values = [0, -0.5]
...
df6 = pd.DataFrame(np.array([[1,5000]]),dtype=float)
df6.insert(0, 'Label', ['Rubber'])
df6.insert(1, 'Type', ['Neo-Hooke'])
df6.to_excel(writer,sheet_name='Materials',index=False)
